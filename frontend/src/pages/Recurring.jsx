    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label className="block text-sm font-bold text-slate-700 mb-2">Next Due</label>
                    <input
                        type="date"
    ...
    return (
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
            <div className="flex justify-between items-center">
                <div>
                    <h2 className="text-3xl font-bold text-slate-900 tracking-tight">Recurring Expenses</h2>
                    <p className="text-slate-500 font-medium mt-1">Automate your fixed monthly bills</p>
                </div>
                <button
            ...
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                {expenses.length === 0 ? (
                    <div className="col-span-full py-16 text-center text-slate-400 bg-white/50 rounded-3xl border border-dashed border-slate-300">
                    ...
                    expenses.map((item) => (
                        <div key={item.id} className="bg-white/80 backdrop-blur-xl p-5 rounded-3xl border border-white/40 shadow-glass group relative overflow-hidden hover:shadow-lg transition-all">
                            <div className="flex justify-between items-start mb-3">
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p className="text-xs text-slate-500 font-medium capitalize">{item.frequency}</p>
                                    </div>
                                </div>
                                <button
                            ...
                            <div className="flex items-end justify-between mt-4">
                                <div>
                                    ...
                                    <p className="text-xl font-extrabold text-slate-900">₹{item.amount.toLocaleString()}</p>
                                </div>
                                <div className="text-right">
                                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider mb-1">Next Due</p>
                                    <div className="flex items-center gap-1 text-sm font-bold text-brand-600 bg-brand-50 px-2 py-1 rounded-lg">
                                        <Calendar size={14} />
